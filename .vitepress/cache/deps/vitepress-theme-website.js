...
var $ = ({ serverURL: e2, lang: n2, path: o2, page: a, pageSize: t2, sortBy: r2, signal: h3, token: i }) => {
  const l3 = {};
  return i && (l3.Authorization = `Bearer ${i}`), fetch(`${s(e2)}comment?path=${encodeURIComponent(o2)}&pageSize=${t2}&page=${a}&lang=${n2}&sortBy=${r2}`, { signal: h3, headers: l3 }).then((g) => g.json()).then((g) => c(g, "Get comment data").data);
};
...
var N = { ...X, emStrongRDelimAst: Ze, emStrongLDelim: ve, url: h2(/^((?:ftp|https?):\/\/|www\.)(?:[a-zA-Z0-9\-]+\.?)+[^\s<]*|^email/, "i").replace("email", /[A-Za-z0-9._+-]+(@)[a-zA-Z0-9-_]+(?:\.[a-zA-Z0-9-_]*[a-zA-Z0-9])+(?![-_])/).getRegex(), _backpedal: /(?:[^?!.,:;*_'"~()&]+|\([^)]*\)|&(?![a-zA-Z0-9]+;$)|[?!.,:;*_'"~)]+(?!$))+/, del: /^(~~?)(?=[^\s~])((?:\\.|[^\\])*?(?:\\.|[^\s~\\]))\1(?=[^~]|$)/, text: /^([`~]+|[^`~])(?:(?= {2,}\n)|(?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)|[\s\S]*?(?:(?=[\\<!\[`*~_]|\b_|https?:\/\/|ftp:\/\/|www\.|$)|[^ ](?= {2,}\n)|[^a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-](?=[a-zA-Z0-9.!#$%&'*+\/=?_`{\|}~-]+@)))/ };
...
`), r2 = "", i = "", s2 = [];
      for (; n2.length > 0; ) {
        let o2 = false, a = [], u2;
        for (u2 = 0; u2 < n2.length; u2++) if (this.rules.other.blockquoteStart.test(n2[u2])) a.push(n2[u2]), o2 = true;
        else if (!o2) a.push(n2[u2]);
        else break;
        n2 = n2.slice(u2);
        let p2 = a.join(`
...
${c2}` : c2;
        ...
        if (this.lexer.state.top = true, this.lexer.blockTokens(c2, s2, true), this.lexer.state.top = f2, n2.length === 0) break;
        let k = s2.at(-1);
...
`, 1)[0], x = !f2.trim(), g = 0;
        if (this.options.pedantic ? (g = 2, c2 = f2.trimStart()) : x ? g = t2[1].length + 1 : (g = t2[2].search(this.rules.other.nonSpaceChar), g = g > 4 ? 1 : g, c2 = f2.slice(g), g += t2[1].length), x && this.rules.other.blankLine.test(k) && (p2 += k + `
`, e2 = e2.substring(k.length + 1), u2 = true), !u2) {
...
`, e2 = e2.substring(G.length + 1), f2 = A.slice(g);
          }
        }
      ...
      for (let u2 = 0; u2 < i.items.length; u2++) if (this.lexer.state.top = false, i.items[u2].tokens = this.lexer.blockTokens(i.items[u2].text, []), !i.loose) {
        let p2 = i.items[u2].tokens.filter((f2) => f2.type === "space"), c2 = p2.length > 0 && p2.some((f2) => this.rules.other.anyLine.test(f2.raw));
        i.loose = c2;
      }
...
`) : [], s2 = { type: "table", raw: t2[0], header: [], align: [], rows: [] };
    if (n2.length === r2.length) {
      ...
      for (let o2 of i) s2.rows.push(V(o2, s2.header.length).map((a, u2) => ({ text: a, tokens: this.lexer.inline(a), header: false, align: s2.align[u2] })));
      return s2;
    }
  }
  lheading(e2) {
...
` ? t2[1].slice(0, -1) : t2[1];
  ...
  emStrong(e2, t2, n2 = "") {
    ...
    if (!(r2[1] || r2[2] || "") || !n2 || this.rules.inline.punctuation.exec(n2)) {
      let s2 = [...r2[0]].length - 1, o2, a, u2 = s2, p2 = 0, c2 = r2[0][0] === "*" ? this.rules.inline.emStrongRDelimAst : this.rules.inline.emStrongRDelimUnd;
      for (c2.lastIndex = 0, t2 = t2.slice(-1 * e2.length + s2); (r2 = c2.exec(t2)) != null; ) {
        ...
        a = Math.min(a, a + u2 + p2);
        let f2 = [...r2[0]][0].length, k = e2.slice(0, s2 + r2.index + f2 + a);
        if (Math.min(s2, a) % 2) {
          let g = k.slice(1, -1);
...
` + r2.raw, this.inlineQueue.at(-1).src = s2.text) : this.tokens.links[r2.tag] || (this.tokens.links[r2.tag] = { href: r2.href, title: r2.title }, t2.push(r2));
      ...
      if ((_c = this.options.extensions) == null ? void 0 : _c.startBlock) {
        ...
        this.options.extensions.startBlock.forEach((u2) => {
          a = u2.call({ lexer: this }, o2), typeof a == "number" && a >= 0 && (s2 = Math.min(s2, a));
...
` + r2.text, this.inlineQueue.pop(), this.inlineQueue.at(-1).src = s2.text) : t2.push(r2);
  ...
  inlineTokens(e2, t2 = []) {
    ...
    for (; e2; ) {
      i || (s2 = ""), i = false;
      let o2;
      if ((_b = (_a3 = this.options.extensions) == null ? void 0 : _a3.inline) == null ? void 0 : _b.some((u2) => (o2 = u2.call({ lexer: this }, e2, t2)) ? (e2 = e2.substring(o2.raw.length), t2.push(o2), true) : false)) continue;
      ...
      if ((_c = this.options.extensions) == null ? void 0 : _c.startInline) {
        let u2 = 1 / 0, p2 = e2.slice(1), c2;
        this.options.extensions.startInline.forEach((f2) => {
          c2 = f2.call({ lexer: this }, p2), typeof c2 == "number" && c2 >= 0 && (u2 = Math.min(u2, c2));
        }), u2 < 1 / 0 && u2 >= 0 && (a = e2.substring(0, u2 + 1));
      }
...
`;
  ...
  listitem(e2) {
    ...
    if (e2.task) {
      ...
      e2.loose ? ((_a3 = e2.tokens[0]) == null ? void 0 : _a3.type) === "paragraph" ? (e2.tokens[0].text = n2 + " " + e2.tokens[0].text, e2.tokens[0].tokens && e2.tokens[0].tokens.length > 0 && e2.tokens[0].tokens[0].type === "text" && (e2.tokens[0].tokens[0].text = n2 + " " + w(e2.tokens[0].tokens[0].text), e2.tokens[0].tokens[0].escaped = true)) : e2.tokens.unshift({ type: "text", raw: n2 + " ", text: n2 + " ", escaped: true }) : t2 += n2 + " ";
    }
...
var B = class {
    ...
    return e2.forEach((n2) => {
      ...
      if (r2.async = this.defaults.async || r2.async || false, n2.extensions && (n2.extensions.forEach((i) => {
        ...
        for (let s2 in n2.renderer) {
          ...
          let o2 = s2, a = n2.renderer[o2], u2 = i[o2];
          i[o2] = (...p2) => {
            let c2 = a.apply(i, p2);
            return c2 === false && (c2 = u2.apply(i, p2)), c2 || "";
          };
        }
        r2.renderer = i;
      }
      if (n2.tokenizer) {
        ...
        for (let s2 in n2.tokenizer) {
          ...
          let o2 = s2, a = n2.tokenizer[o2], u2 = i[o2];
          i[o2] = (...p2) => {
            let c2 = a.apply(i, p2);
            return c2 === false && (c2 = u2.apply(i, p2)), c2;
          };
      ...
      if (n2.hooks) {
        ...
        for (let s2 in n2.hooks) {
          ...
          let o2 = s2, a = n2.hooks[o2], u2 = i[o2];
          $2.passThroughHooks.has(s2) ? i[o2] = (p2) => {
            if (this.defaults.async) return Promise.resolve(a.call(i, p2)).then((f2) => u2.call(i, f2));
            let c2 = a.call(i, p2);
            return u2.call(i, c2);
          } : i[o2] = (...p2) => {
            let c2 = a.apply(i, p2);
  ...
  parseMarkdown(e2) {
    return (n2, r2) => {
      ...
      let a = s2.hooks ? s2.hooks.provideLexer() : e2 ? b.lex : b.lexInline, u2 = s2.hooks ? s2.hooks.provideParser() : e2 ? R2.parse : R2.parseInline;
      if (s2.async) return Promise.resolve(s2.hooks ? s2.hooks.preprocess(n2) : n2).then((p2) => a(p2, s2)).then((p2) => s2.hooks ? s2.hooks.processAllTokens(p2) : p2).then((p2) => s2.walkTokens ? Promise.all(this.walkTokens(p2, s2.walkTokens)).then(() => p2) : p2).then((p2) => u2(p2, s2)).then((p2) => s2.hooks ? s2.hooks.postprocess(p2) : p2).catch(o2);
      try {
...
var na = (e2) => {
  ...
  return e2.replace(la, (t2, a, n2) => {
    ...
    const h3 = `<span style="color: #${s2}">${a}</span>`;
    return l3 = ++l3 % qe2.length, h3;
  });
};
...
var ma = ({ serverURL: e2, path: l3 = location.pathname, lang: t2 = typeof navigator > "u" ? "en-US" : navigator.language, locale: a, meta: n2 = ["nick", "mail", "link"], requiredMeta: s2 = [], dark: h3 = false, pageSize: c2 = 10, wordLimit: f2, noCopyright: v3 = false, login: y3 = "enable", recaptchaV3Key: m3 = "", turnstileKey: R3 = "", commentSorting: j3 = "latest", emoji: E2 = Pe2, imageUploader: $3, highlighter: S2, texRenderer: i, search: g, reaction: K2, ...X2 }) => ({ serverURL: ye2(e2), path: lt(l3), lang: tt(t2), locale: { ...et(tt(t2)), ...typeof a == "object" ? a : {} }, wordLimit: ua(f2), meta: De2(n2), requiredMeta: De2(s2), dark: h3, pageSize: c2, commentSorting: j3, login: y3, noCopyright: v3, recaptchaV3Key: m3, turnstileKey: R3, ...X2, reaction: K2 === true ? Zt2 : K2 || null, imageUploader: ce2($3, Xt), highlighter: ce2(S2, na), texRenderer: ce2(i, Yt), emoji: ce2(E2, Pe2), search: ce2(g, Jt(t2)) });
...
var pa = (e2, l3, t2) => {
  ...
  if (s2 === 0) {
    ...
    if (c2 === 0) {
      ...
      if (v3 === 0) {
        const y3 = f2 % 6e4;
        return `${Math.round(y3 / 1e3)} ${t2.seconds}`;
      }
...
var ba = (e2) => Promise.all(e2 ? e2.map((l3) => re2(l3) ? ya(nt(l3)) : Promise.resolve(l3)) : []).then((l3) => {
  const t2 = { tabs: [], map: {} };
  return l3.forEach((a) => {
    const { name: n2, folder: s2, icon: h3, prefix: c2 = "", type: f2, items: v3 } = a;
    t2.tabs.push({ name: n2, icon: it(h3, s2, c2, f2), items: v3.map((y3) => {
      const m3 = `${c2}${y3}`;
      return t2.map[m3] = it(y3, s2, c2, f2), m3;
    }) });
  }), t2;
});
var st = (e2) => {
...
var mt = ({ serverURL: e2, path: l3 = window.location.pathname, selector: t2 = ".waline-comment-count", lang: a = navigator.language }) => {
  ...
  return s2.length && f({ serverURL: ye2(e2), paths: Array.from(s2).map((h3) => lt(Re2(h3) ?? l3)), lang: a, signal: n2.signal }).then((h3) => {
    s2.forEach((c2, f2) => {
...
var ol = defineComponent({ __name: "ArticleReaction", setup(e2) {
  const l3 = Ga(), t2 = inject(we2), a = ref(-1), n2 = ref([]), s2 = computed(() => t2.value.locale), h3 = computed(() => {
    const { reaction: m3 } = t2.value;
    return m3 != null && m3.length ? m3 : null;
  }), c2 = computed(() => {
    var m3;
    const { path: R3 } = t2.value;
    return ((m3 = h3.value) == null ? void 0 : m3.map((j3, E2) => ({ icon: j3, desc: s2.value[`reaction${E2}`], active: l3.value[R3] === E2 }))) ?? null;
  });
  let f2;
  const v3 = async () => {
    ...
    f2 = E2.abort.bind(E2);
    const [$3] = await p({ serverURL: m3, lang: R3, paths: [j3], type: h3.value.map((S2, i) => `reaction${i}`), signal: E2.signal });
    n2.value = h3.value.map((S2, i) => $3[`reaction${i}`]);
  }, y3 = async (m3) => {
    if (a.value !== -1) return;
    const { serverURL: R3, lang: j3, path: E2 } = t2.value, $3 = l3.value[E2];
    a.value = m3, $3 !== void 0 && (await d({ serverURL: R3, lang: j3, path: E2, type: `reaction${$3}`, action: "desc" }), n2.value[$3] = Math.max(n2.value[$3] - 1, 0)), $3 !== m3 && (await d({ serverURL: R3, lang: j3, path: E2, type: `reaction${m3}` }), n2.value[m3] = (n2.value[m3] || 0) + 1), $3 === m3 ? delete l3.value[E2] : l3.value[E2] = m3, a.value = -1;
  };
  return onMounted(() => {
    watchImmediate(() => [t2.value.serverURL, t2.value.path], () => v3());
  }), onUnmounted(() => {
    f2 == null || f2();
  }), (m3, R3) => c2.value ? (openBlock(), createElementBlock("div", Ja, [createBaseVNode("div", { class: "wl-reaction-title", textContent: toDisplayString(s2.value.reactionTitle) }, null, 8, Qa), createBaseVNode("ul", el, [(openBlock(true), createElementBlock(Fragment, null, renderList(c2.value, ({ active: j3, icon: E2, desc: $3 }, S2) => (openBlock(), createElementBlock("li", { key: S2, class: normalizeClass(["wl-reaction-item", { active: j3 }]), onClick: (i) => y3(S2) }, [createBaseVNode("div", al, [createBaseVNode("img", { src: E2, alt: $3 }, null, 8, ll), a.value === S2 ? (openBlock(), createBlock(unref(ue2), { key: 0, class: "wl-reaction-loading" })) : (openBlock(), createElementBlock("div", { key: 1, class: "wl-reaction-votes", textContent: toDisplayString(n2.value[S2] || 0) }, null, 8, nl))]), createBaseVNode("div", { class: "wl-reaction-text", textContent: toDisplayString($3) }, null, 8, rl)], 10, tl))), 128))])])) : createCommentVNode("v-if", true);
} });
var il = ["data-index"];
...
var cl = defineComponent({ __name: "ImageWall", props: { items: { default: () => [] }, columnWidth: { default: 300 }, gap: { default: 0 } }, emits: ["insert"], setup(e2) {
  const l3 = e2;
  let t2 = null;
  const a = useTemplateRef("wall"), n2 = ref({}), s2 = ref([]), h3 = () => {
    const m3 = Math.floor((a.value.getBoundingClientRect().width + l3.gap) / (l3.columnWidth + l3.gap));
    return m3 > 0 ? m3 : 1;
  }, c2 = (m3) => new Array(m3).fill(null).map(() => []), f2 = async (m3) => {
    var R3;
    if (m3 >= l3.items.length) return;
    await nextTick();
    const j3 = Array.from(((R3 = a.value) == null ? void 0 : R3.children) ?? []).reduce((E2, $3) => $3.getBoundingClientRect().height < E2.getBoundingClientRect().height ? $3 : E2);
  ...
  }), (m3, R3) => (openBlock(), createElementBlock("div", { ref_key: "wall", ref: a, class: "wl-gallery", style: normalizeStyle({ gap: `${m3.gap}px` }) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(s2.value, (j3, E2) => (openBlock(), createElementBlock("div", { key: E2, class: "wl-gallery-column", "data-index": E2, style: normalizeStyle({ gap: `${m3.gap}px` }) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(j3, ($3) => (openBlock(), createElementBlock(Fragment, { key: $3 }, [n2.value[m3.items[$3].src] ? createCommentVNode("v-if", true) : (openBlock(), createBlock(unref(ue2), { key: 0, size: 36, style: { margin: "20px auto" } })), createBaseVNode("img", { class: "wl-gallery-item", src: m3.items[$3].src, title: m3.items[$3].title, loading: "lazy", onLoad: y3, onClick: (S2) => m3.$emit("insert", `![](${m3.items[$3].src})`) }, null, 40, sl)], 64))), 128))], 12, il))), 128))], 4));
} });
...
var gt = defineComponent({ __name: "CommentBox", props: { edit: {}, rootId: {}, replyId: {}, replyUser: {} }, emits: ["log", "cancelEdit", "cancelReply", "submit"], setup(e2, { emit: l3 }) {
  const t2 = e2, a = l3, n2 = inject(we2), s2 = Ba(), h3 = Wa(), c2 = be2(), f2 = ref({}), v3 = useTemplateRef("textarea"), y3 = useTemplateRef("image-uploader"), m3 = useTemplateRef("emoji-button"), R3 = useTemplateRef("emoji-popup"), j3 = useTemplateRef("gif-button"), E2 = useTemplateRef("gif-popup"), $3 = useTemplateRef("gif-search"), S2 = ref({ tabs: [], map: {} }), i = ref(0), g = ref(false), K2 = ref(false), X2 = ref(false), A = ref(""), V2 = ref(0), T = reactive({ loading: true, list: [] }), le2 = ref(0), Q2 = ref(false), me = ref(""), w3 = ref(false), U3 = ref(false), b2 = computed(() => n2.value.locale), F2 = computed(() => !!c2.value.token), q2 = computed(() => n2.value.imageUploader !== null), O2 = (d3) => {
    const r2 = v3.value, C3 = r2.selectionStart, H2 = r2.selectionEnd || 0, k = r2.scrollTop;
    s2.value = r2.value.substring(0, C3) + d3 + r2.value.substring(H2, r2.value.length), r2.focus(), r2.selectionStart = C3 + d3.length, r2.selectionEnd = C3 + d3.length, r2.scrollTop = k;
  }, oe2 = ({ key: d3, ctrlKey: r2, metaKey: C3 }) => {
    w3.value || (r2 || C3) && d3 === "Enter" && Se2();
  }, ve2 = async (d3) => {
    const r2 = `![${n2.value.locale.uploading} ${d3.name}]()`;
    O2(r2), w3.value = true;
    try {
      const C3 = await n2.value.imageUploader(d3);
...
![${d3.name}](${C3})`);
    ...
    var r2;
    if ((r2 = d3.dataTransfer) != null && r2.items) {
      const C3 = ct(d3.dataTransfer.items);
      C3 && q2.value && (ve2(C3), d3.preventDefault());
    }
  }, yt = (d3) => {
    if (d3.clipboardData) {
      const r2 = ct(d3.clipboardData.items);
      r2 && q2.value && ve2(r2);
    }
  }, bt = () => {
    const d3 = y3.value;
    d3.files && q2.value && ve2(d3.files[0]).then(() => {
      d3.value = "";
    });
  }, Se2 = async () => {
    var d3;
    const { serverURL: r2, lang: C3, login: H2, wordLimit: k, requiredMeta: D2, recaptchaV3Key: N2, turnstileKey: Y } = n2.value, _2 = { comment: me.value, nick: h3.value.nick, mail: h3.value.mail, link: h3.value.link, url: n2.value.path, ua: await Ra() };
    ...
    if (!Q2.value) {
      alert(b2.value.wordHint.replace("$0", k[0].toString()).replace("$1", k[1].toString()).replace("$2", V2.value.toString()));
      return;
    }
    ...
    try {
      ...
      a("submit", Ce2.data), s2.value = "", A.value = "", await nextTick(), t2.replyId && a("cancelReply"), (d3 = t2.edit) != null && d3.objectId && a("cancelEdit");
    } catch (de2) {
    ...
    var r2, C3, H2, k;
    !((r2 = m3.value) != null && r2.contains(d3.target)) && !((C3 = R3.value) != null && C3.contains(d3.target)) && (g.value = false), !((H2 = j3.value) != null && H2.contains(d3.target)) && !((k = E2.value) != null && k.contains(d3.target)) && (K2.value = false);
  }, Ue2 = async (d3) => {
    var r2;
    const { scrollTop: C3, clientHeight: H2, scrollHeight: k } = d3.target, D2 = (H2 + C3) / k, N2 = n2.value.search, Y = ((r2 = $3.value) == null ? void 0 : r2.value) ?? "";
    D2 < 0.9 || T.loading || U3.value || (T.loading = true, (N2.more && T.list.length ? await N2.more(Y, T.list.length) : await N2.search(Y)).length ? T.list = [...T.list, ...N2.more && T.list.length ? await N2.more(Y, T.list.length) : await N2.search(Y)] : U3.value = true, T.loading = false, setTimeout(() => {
      d3.target.scrollTop = C3;
    }, 50));
  }, ze2 = useDebounceFn((d3) => {
    T.list = [], U3.value = false, Ue2(d3);
  }, 300);
  return useEventListener("click", $t), useEventListener("message", ({ data: d3 }) => {
    !d3 || d3.type !== "profile" || (c2.value = { ...c2.value, ...d3.data }, [localStorage, sessionStorage].filter((r2) => r2.getItem("WALINE_USER")).forEach((r2) => {
      r2.setItem("WALINE_USER", JSON.stringify(c2));
    }));
  }), watchImmediate([n2, V2], ([d3, r2]) => {
    const { wordLimit: C3 } = d3;
    ...
    $3.value && ($3.value.value = ""), T.loading = true, T.list = await (((r2 = C3.default) == null ? void 0 : r2.call(C3)) ?? C3.search("")), T.loading = false;
  }), onMounted(() => {
    var d3;
    (d3 = t2.edit) != null && d3.objectId && (s2.value = t2.edit.orig), watchImmediate(() => s2.value, (r2) => {
      const { highlighter: C3, texRenderer: H2 } = n2.value;
      me.value = r2, A.value = Ia(r2, { emojiMap: S2.value.map, highlighter: C3, texRenderer: H2 }), V2.value = xa(r2), r2 ? autosize_esm_default(v3.value) : autosize_esm_default.destroy(v3.value);
    }), watchImmediate(() => n2.value.emoji, async (r2) => {
      S2.value = await ba(r2);
    });
  }), (d3, r2) => {
    var C3, H2;
    return openBlock(), createElementBlock("div", { key: unref(c2).token, class: "wl-comment" }, [unref(n2).login !== "disable" && F2.value && !((C3 = d3.edit) != null && C3.objectId) ? (openBlock(), createElementBlock("div", ul, [createBaseVNode("div", ml, [createBaseVNode("button", { type: "submit", class: "wl-logout-btn", title: b2.value.logout, onClick: Ct }, [createVNode(unref(vt), { size: 14 })], 8, vl), createBaseVNode("a", { href: "#", class: "wl-login-nick", "aria-label": "Profile", title: b2.value.profile, onClick: je2 }, [createBaseVNode("img", { src: unref(c2).avatar, alt: "avatar" }, null, 8, pl)], 8, dl)]), createBaseVNode("a", { href: "#", class: "wl-login-nick", "aria-label": "Profile", title: b2.value.profile, onClick: je2, textContent: toDisplayString(unref(c2).display_name) }, null, 8, gl)])) : createCommentVNode("v-if", true), createBaseVNode("div", hl, [unref(n2).login !== "force" && unref(n2).meta.length && !F2.value ? (openBlock(), createElementBlock("div", { key: 0, class: normalizeClass(["wl-header", `item${unref(n2).meta.length}`]) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(unref(n2).meta, (k) => (openBlock(), createElementBlock("div", { key: k, class: "wl-header-item" }, [createBaseVNode("label", { for: `wl-${k}`, textContent: toDisplayString(b2.value[k] + (unref(n2).requiredMeta.includes(k) || !unref(n2).requiredMeta.length ? "" : `(${b2.value.optional})`)) }, null, 8, fl), withDirectives(createBaseVNode("input", { id: `wl-${k}`, ref_for: true, ref: (D2) => {
      D2 && (f2.value[k] = D2);
    }, "onUpdate:modelValue": (D2) => unref(h3)[k] = D2, class: normalizeClass(["wl-input", `wl-${k}`]), name: k, type: k === "mail" ? "email" : "text" }, null, 10, wl), [[vModelDynamic, unref(h3)[k]]])]))), 128))], 2)) : createCommentVNode("v-if", true), withDirectives(createBaseVNode("textarea", { id: "wl-edit", ref: "textarea", "onUpdate:modelValue": r2[0] || (r2[0] = (k) => isRef(s2) ? s2.value = k : null), class: "wl-editor", placeholder: d3.replyUser ? `@${d3.replyUser}` : b2.value.placeholder, onKeydown: oe2, onDrop: ke2, onPaste: yt }, null, 40, yl), [[vModelText, unref(s2)]]), withDirectives(createBaseVNode("div", bl, [r2[7] || (r2[7] = createBaseVNode("hr", null, null, -1)), createBaseVNode("h4", null, toDisplayString(b2.value.preview) + ":", 1), createBaseVNode("div", { class: "wl-content", innerHTML: A.value }, null, 8, kl)], 512), [[vShow, X2.value]]), createBaseVNode("div", Cl, [createBaseVNode("div", $l, [createBaseVNode("a", Ll, [createVNode(unref(_a2))]), withDirectives(createBaseVNode("button", { ref: "emoji-button", type: "button", class: normalizeClass(["wl-action", { active: g.value }]), title: b2.value.emoji, onClick: r2[1] || (r2[1] = (k) => g.value = !g.value) }, [createVNode(unref(ja))], 10, El), [[vShow, S2.value.tabs.length]]), unref(n2).search ? (openBlock(), createElementBlock("button", { key: 0, ref: "gif-button", type: "button", class: normalizeClass(["wl-action", { active: K2.value }]), title: b2.value.gif, onClick: r2[2] || (r2[2] = (k) => K2.value = !K2.value) }, [createVNode(unref(Na))], 10, Il)) : createCommentVNode("v-if", true), createBaseVNode("input", { id: "wl-image-upload", ref: "image-uploader", class: "upload", "aria-hidden": "true", type: "file", accept: ".png,.jpg,.jpeg,.webp,.bmp,.gif", onChange: bt }, null, 544), q2.value ? (openBlock(), createElementBlock("label", { key: 1, for: "wl-image-upload", class: "wl-action", title: b2.value.uploadImage, "aria-label": b2.value.uploadImage }, [createVNode(unref(Ua))], 8, Al)) : createCommentVNode("v-if", true), createBaseVNode("button", { type: "button", class: normalizeClass(["wl-action", { active: X2.value }]), title: b2.value.preview, onClick: r2[3] || (r2[3] = (k) => X2.value = !X2.value) }, [createVNode(unref(Va))], 10, Ml)]), createBaseVNode("div", xl, [r2[9] || (r2[9] = createBaseVNode("div", { class: "wl-captcha-container" }, null, -1)), createBaseVNode("div", Rl, [createTextVNode(toDisplayString(V2.value) + " ", 1), unref(n2).wordLimit ? (openBlock(), createElementBlock("span", Sl, [r2[8] || (r2[8] = createTextVNode("  /  ")), createBaseVNode("span", { class: normalizeClass({ illegal: !Q2.value }), textContent: toDisplayString(le2.value) }, null, 10, jl)])) : createCommentVNode("v-if", true), createTextVNode("  " + toDisplayString(b2.value.word), 1)]), unref(n2).login !== "disable" && !F2.value ? (openBlock(), createElementBlock("button", { key: 0, type: "button", class: "wl-btn", onClick: kt, textContent: toDisplayString(b2.value.login) }, null, 8, Ul)) : createCommentVNode("v-if", true), unref(n2).login !== "force" || F2.value ? (openBlock(), createElementBlock("button", { key: 1, type: "submit", class: "primary wl-btn", title: "Cmd|Ctrl + Enter", disabled: w3.value, onClick: Se2 }, [w3.value ? (openBlock(), createBlock(unref(ue2), { key: 0, size: 16 })) : (openBlock(), createElementBlock(Fragment, { key: 1 }, [createTextVNode(toDisplayString(b2.value.submit), 1)], 64))], 8, zl)) : createCommentVNode("v-if", true)]), createBaseVNode("div", { ref: "gif-popup", class: normalizeClass(["wl-gif-popup", { display: K2.value }]) }, [createBaseVNode("input", { ref: "gif-search", type: "text", placeholder: b2.value.gifSearchPlaceholder, onInput: r2[4] || (r2[4] = (...k) => unref(ze2) && unref(ze2)(...k)) }, null, 40, Vl), T.list.length ? (openBlock(), createBlock(cl, { key: 0, items: T.list, "column-width": 200, gap: 6, onInsert: r2[5] || (r2[5] = (k) => O2(k)), onScroll: Ue2 }, null, 8, ["items"])) : createCommentVNode("v-if", true), T.loading ? (openBlock(), createElementBlock("div", _l, [createVNode(unref(ue2), { size: 30 })])) : createCommentVNode("v-if", true)], 2), createBaseVNode("div", { ref: "emoji-popup", class: normalizeClass(["wl-emoji-popup", { display: g.value }]) }, [(openBlock(true), createElementBlock(Fragment, null, renderList(S2.value.tabs, (k, D2) => (openBlock(), createElementBlock(Fragment, { key: k.name }, [D2 === i.value ? (openBlock(), createElementBlock("div", Hl, [(openBlock(true), createElementBlock(Fragment, null, renderList(k.items, (N2) => (openBlock(), createElementBlock("button", { key: N2, type: "button", title: N2, onClick: (Y) => O2(`:${N2}:`) }, [g.value ? (openBlock(), createElementBlock("img", { key: 0, class: "wl-emoji", src: S2.value.map[N2], alt: N2, loading: "lazy", referrerPolicy: "no-referrer" }, null, 8, Fl)) : createCommentVNode("v-if", true)], 8, Tl))), 128))])) : createCommentVNode("v-if", true)], 64))), 128)), S2.value.tabs.length > 1 ? (openBlock(), createElementBlock("div", Nl, [(openBlock(true), createElementBlock(Fragment, null, renderList(S2.value.tabs, (k, D2) => (openBlock(), createElementBlock("button", { key: k.name, type: "button", class: normalizeClass(["wl-tab", { active: i.value === D2 }]), onClick: (N2) => i.value = D2 }, [createBaseVNode("img", { class: "wl-emoji", src: k.icon, alt: k.name, title: k.name, loading: "lazy", referrerPolicy: "no-referrer" }, null, 8, Bl)], 10, Wl))), 128))])) : createCommentVNode("v-if", true)], 2)])]), d3.replyId || (H2 = d3.edit) != null && H2.objectId ? (openBlock(), createElementBlock("button", { key: 1, type: "button", class: "wl-close", title: b2.value.cancelReply, onClick: r2[6] || (r2[6] = (k) => d3.replyId ? a("cancelReply") : a("cancelEdit")) }, [createVNode(unref(vt), { size: 24 })], 8, Dl)) : createCommentVNode("v-if", true)]);
  };
} });
var Pl = ["id"];
...
var hn = defineComponent({ __name: "CommentCard", props: { comment: {}, edit: {}, rootId: {}, reply: {} }, emits: ["log", "submit", "delete", "like", "sticky", "edit", "reply", "status"], setup(e2, { emit: l3 }) {
  const t2 = e2, a = l3, n2 = ["approved", "waiting", "spam"], s2 = inject(we2), h3 = dt(), c2 = useNow(), f2 = be2(), v3 = computed(() => s2.value.locale), y3 = computed(() => {
    const { link: i } = t2.comment;
    return i ? rt(i) ? i : `https://${i}` : "";
  }), m3 = computed(() => h3.value.includes(t2.comment.objectId)), R3 = computed(() => pa(new Date(t2.comment.time), c2.value, v3.value)), j3 = computed(() => f2.value.type === "administrator"), E2 = computed(() => t2.comment.user_id && f2.value.objectId === t2.comment.user_id), $3 = computed(() => {
    var i;
...
var Sn = defineComponent({ __name: "WalineComment", props: { serverURL: {}, path: {}, meta: {}, requiredMeta: {}, wordLimit: {}, pageSize: {}, lang: {}, locale: {}, commentSorting: {}, dark: { type: [String, Boolean] }, login: {}, noCopyright: { type: Boolean }, recaptchaV3Key: {}, turnstileKey: {}, reaction: { type: [Array, Boolean] }, emoji: {}, search: {}, highlighter: { type: Function }, imageUploader: { type: Function }, texRenderer: { type: Function } }, setup(e2) {
  const l3 = e2, t2 = be2(), a = dt(), n2 = ref("loading"), s2 = ref(0), h3 = ref(1), c2 = ref(0), f2 = computed(() => ma(l3)), v3 = ref(f2.value.commentSorting), y3 = ref([]), m3 = ref(null), R3 = ref(null), j3 = computed(() => va(f2.value.dark)), E2 = computed(() => f2.value.locale);
  ...
  const S2 = (w3) => {
    ...
    n2.value = "loading", $3 == null || $3(), $({ serverURL: U3, lang: f2.value.lang, path: b2, pageSize: F2, sortBy: at[v3.value], page: w3, signal: q2.signal, token: t2.value.token }).then((O2) => {
      n2.value = "success", s2.value = O2.count, y3.value.push(...O2.data), h3.value = w3, c2.value = O2.totalPages;
    }).catch((O2) => {
    ...
    if (R3.value) R3.value.comment = w3.comment, R3.value.orig = w3.orig;
      ...
      const U3 = y3.value.find(({ objectId: b2 }) => b2 === w3.rid);
      if (!U3) return;
      Array.isArray(U3.children) || (U3.children = []), U3.children.push(w3);
    } else y3.value.unshift(w3), s2.value += 1;
  }, T = async ({ comment: w3, status: U3 }) => {
    ...
    const { serverURL: U3, lang: b2 } = f2.value;
    await y({ serverURL: U3, lang: b2, token: t2.value.token, objectId: w3 }), y3.value.some((F2, q2) => F2.objectId === w3 ? (y3.value = y3.value.filter((O2, oe2) => oe2 !== q2), true) : F2.children.some((O2, oe2) => O2.objectId === w3 ? (y3.value[q2].children = F2.children.filter((ve2, ke2) => ke2 !== oe2), true) : false));
  }, me = async (w3) => {
    const { serverURL: U3, lang: b2 } = f2.value, { objectId: F2 } = w3, q2 = a.value.includes(F2);
    await U({ serverURL: U3, lang: b2, objectId: F2, token: t2.value.token, comment: { like: !q2 } }), q2 ? a.value = a.value.filter((O2) => O2 !== F2) : (a.value = [...a.value, F2], a.value.length > 50 && (a.value = a.value.slice(-50))), w3.like = Math.max(0, (w3.like || 0) + (q2 ? -1 : 1));
  };
  return provide(we2, f2), onMounted(() => {
    ...
    $3 == null || $3();
  }), (w3, U3) => (openBlock(), createElementBlock("div", fn, [createVNode(ol), !m3.value && !R3.value ? (openBlock(), createBlock(gt, { key: 0, onLog: g, onSubmit: V2 })) : createCommentVNode("v-if", true), createBaseVNode("div", wn, [createBaseVNode("div", yn, [s2.value ? (openBlock(), createElementBlock("span", { key: 0, class: "wl-num", textContent: toDisplayString(s2.value) }, null, 8, bn)) : createCommentVNode("v-if", true), createTextVNode(" " + toDisplayString(E2.value.comment), 1)]), createBaseVNode("ul", kn, [(openBlock(true), createElementBlock(Fragment, null, renderList(unref(ca), (b2) => (openBlock(), createElementBlock("li", { key: b2, class: normalizeClass([b2 === v3.value ? "active" : ""]), onClick: (F2) => K2(b2) }, toDisplayString(E2.value[b2]), 11, Cn))), 128))])]), createBaseVNode("div", $n, [(openBlock(true), createElementBlock(Fragment, null, renderList(y3.value, (b2) => (openBlock(), createBlock(hn, { key: b2.objectId, "root-id": b2.objectId, comment: b2, reply: m3.value, edit: R3.value, onLog: g, onReply: X2, onEdit: A, onSubmit: V2, onStatus: T, onDelete: Q2, onSticky: le2, onLike: me }, null, 8, ["root-id", "comment", "reply", "edit"]))), 128))]), n2.value === "error" ? (openBlock(), createElementBlock("div", Ln, [createBaseVNode("button", { type: "button", class: "wl-btn", onClick: g, textContent: toDisplayString(E2.value.refresh) }, null, 8, En)])) : n2.value === "loading" ? (openBlock(), createElementBlock("div", In, [createVNode(unref(ue2), { size: 30 })])) : y3.value.length ? h3.value < c2.value ? (openBlock(), createElementBlock("div", Mn, [createBaseVNode("button", { type: "button", class: "wl-btn", onClick: i, textContent: toDisplayString(E2.value.more) }, null, 8, xn)])) : createCommentVNode("v-if", true) : (openBlock(), createElementBlock("div", { key: 3, class: "wl-empty", textContent: toDisplayString(E2.value.sofa) }, null, 8, An)), f2.value.noCopyright ? createCommentVNode("v-if", true) : (openBlock(), createElementBlock("div", Rn, [U3[0] || (U3[0] = createTextVNode(" Powered by ")), U3[1] || (U3[1] = createBaseVNode("a", { href: "https://github.com/walinejs/waline", target: "_blank", rel: "noopener noreferrer" }, " Waline ", -1)), createTextVNode(" v" + toDisplayString(unref(ht)), 1)]))]));
} });
var ft = (e2, l3) => {
  l3.forEach((t2, a) => {
    const n2 = e2[a].time;
...
var wt = ({ serverURL: e2, path: l3 = window.location.pathname, selector: t2 = ".waline-pageview-count", update: a = true, lang: n2 = navigator.language }) => {
  const s2 = new AbortController(), h3 = Array.from(document.querySelectorAll(t2)), c2 = (v3) => {
    const y3 = Re2(v3);
    return y3 !== null && l3 !== y3;
  }, f2 = (v3) => j({ serverURL: ye2(e2), paths: v3.map((y3) => Re2(y3) ?? l3), lang: n2, signal: s2.signal }).then((y3) => ft(y3, v3)).catch(st);
  if (a) {
    const v3 = h3.filter((m3) => !c2(m3)), y3 = h3.filter(c2);
    v({ serverURL: ye2(e2), path: l3, lang: n2 }).then((m3) => ft(m3, v3)), y3.length && f2(y3);
  } else f2(h3);
...
var jn = ({ el: e2 = "#waline", path: l3 = window.location.pathname, comment: t2 = false, pageview: a = false, ...n2 }) => {
  ...
  const h3 = reactive({ ...n2 }), c2 = reactive({ comment: t2, pageview: a, path: l3 }), f2 = () => {
  ...
  let y3 = null;
  s2 && (y3 = createApp(() => h(Sn, { path: c2.path, ...h3 })), y3.mount(s2));
  const m3 = watchEffect(f2), R3 = watchEffect(v3);
  return { el: s2, update: ({ comment: j3, pageview: E2, path: $3 = window.location.pathname, ...S2 } = {}) => {
    Object.entries(S2).forEach(([i, g]) => {
      h3[i] = g;
    }), c2.path = $3, j3 !== void 0 && (c2.comment = j3), E2 !== void 0 && (c2.pageview = E2);
  }, destroy: () => {
    y3 == null || y3.unmount(), m3(), R3();
  } };
};

// node_modules/vitepress-theme-website/dist/waline/components/Waline.mjs
import "D:/Users/Desktop/Project/Cynara_Education/node_modules/@waline/client/dist/waline.css";
...
import Live2d from "D:/Users/Desktop/Project/Cynara_Education/node_modules/vitepress-theme-website/dist/live2d/components/Live2dComponent.vue";
Live2d.init = (props) => {
  const container = document.createElement("div");
